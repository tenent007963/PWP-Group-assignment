...
def delmanAssign():
    ...
    try:
        ...
        if oid & udt.index(delman):
            confirm = input(print('Confirm assigning ', order, 'to ', delman,'?\n'))
            if confirm == 'Y':
                dt = getrow(odb, oid)
                dt[9] = delman
...
def placeOrder(u, items = [], trm = 0):
    pdb = 'products.txt'
    sel = input(print('Select your item using product ID:'))
    listallItem()
    try:
        pdt = getrow(pdb, sel)
        if pdt:
            qtt = input(print('Please enter order quantity for this product:'))
            if int(qtt) <= int(pdt[3]):
                rm = int(pdt[5]) * int(qtt)
...
def addtoCart(usr, pid, qtt, rm, items, trm, opt = 0):
    ...
    tlst = [pid, qtt]
    try:
        opt = int(input(print('Do you want to checkout now or continue shopping?\n1. Continue\n2. Checkout\n3. Discard cart'))) if opt == 0 else opt
        if opt == 1:
            ...
            confirm = input(print('Are you sure to discard cart? Once done no revert can be done(Y/n)):'))
            if confirm == "Y":
...
def makePayment(u):
    ...
    try:
        ...
        opt = input(print('Please enter orderID to select order and pay:'))
        if opt:
            dt2 = getrow(odb, opt)
            tpc = input(print('Please enter your TPCard number:'))
            confirm = input(print('Amount ', dt2[5], 'will be charged from card ', tpc, '.\nPlease confirm (Y/n):'))
...
def theMagic(bstring):
    ...
    for item in bstring:
        if item.find(':') :
            if item.find('name'):
                titem = input(print(item))
                lst.append(titem) if datavalidate(titem, 1) != 0 else input(print(item))
                c += 1
            elif item.find('email'):
                titem = input(print(item))
                lst.append(titem) if datavalidate(titem, 2) != 0 else input(print(item))
                c += 1
            elif item.find('phone'):
                titem = input(print(item))
                lst.append(titem) if datavalidate(titem, 3) != 0 else input(print(item))
                c += 1
            elif item.find('address'):
                titem = input(print(item))
                lst.append(titem) if datavalidate(titem, 4) != 0 else input(print(item))
                c += 1
            else:
                lst.append(titem)
                c += 1
